"""
Research-grade benchmarking experiment runner for MCP protocol.

Systematically tests MCP API performance across multiple concurrency levels
with proper experimental controls and statistical rigor.

Uses MCP Resources (not Tools) for data retrieval, following proper MCP semantics:
- Resources: For reading existing entities by identifier (modelcard://id)
- Tools: For performing operations/searches
"""
...
class MCPExperimentRunner:
    """Orchestrates systematic benchmarking experiments for MCP protocol."""
    ...
    async def _create_mcp_session(self):
        """Create a new MCP session with SSE transport."""
        ...
        # IMPORTANT: Initialize the session (MCP handshake)
        # This must complete before any tool calls
        await session.initialize()

        return session, transport

    async def _close_mcp_session(self, session, transport):
    ...
    async def warmup(self, num_requests: int):
        """Perform warmup requests to stabilize server state."""
        print(f"  Warmup: {num_requests} requests...", end=" ", flush=True)

        # Create single session for warmup
        session, transport = await self._create_mcp_session()

        try:
            tasks = []
            uri = f"modelcard://{self.config['modelcard_id']}"
            for i in range(num_requests):
                task = session.read_resource(uri)
                tasks.append(task)

            await asyncio.gather(*tasks, return_exceptions=True)
        finally:
            await self._close_mcp_session(session, transport)

        print("✓")

    async def _single_request(self, semaphore: asyncio.Semaphore, request_id: int) -> Dict[str, Any]:
        """Execute a single MCP resource read with timing."""
        async with semaphore:
            # Create a new session for each request to simulate realistic concurrent clients
            session = None
            transport = None
            ...
            try:
                ...
                # Read the MCP resource (proper semantics for data retrieval)
                uri = f"modelcard://{self.config['modelcard_id']}"
                ...
                # Extract actual content from MCP resource result
                # result.contents is a list of content items (TextResourceContents, BlobResourceContents)
                content_texts = []
                for content_item in result.contents:
                    if hasattr(content_item, 'text'):
                        content_texts.append(content_item.text)
                    elif hasattr(content_item, 'blob'):
                        content_texts.append(content_item.blob.decode('utf-8'))

                # Calculate response size based on combined content
                combined_content = ''.join(content_texts)
                response_size_bytes = len(combined_content.encode('utf-8'))
    ...
    async def run_trial(self, concurrency_level: int, trial_num: int) -> Dict[str, Any]:
        ...
        # Warmup phase
        await self.warmup(self.config['warmup_requests'])
        ...
        start_time = time.perf_counter()
        tasks = [
            self._single_request(semaphore, i)
    ...
    def _aggregate_trials(self, trials_data: List[Dict[str, Any]], concurrency_level: int) -> Dict[str, Any]:
        ...
        for metric in metrics:
            ...
            if values:
                ...
                # Default zeros if no data for this metric
                aggregate[f'{metric}_mean'] = 0.0
                aggregate[f'{metric}_stdev'] = 0.0
                aggregate[f'{metric}_min'] = 0.0
                aggregate[f'{metric}_max'] = 0.0

        return aggregate

    def _print_summary(self, aggregate_stats: Dict[str, Any]):
        """Print summary statistics for a concurrency level."""
        print(f"\n  Summary (across {aggregate_stats.get('num_trials', 0)} trials):")
        print(f"    Throughput:        {aggregate_stats.get('throughput_req_per_sec_mean', 0.0):.2f} ± {aggregate_stats.get('throughput_req_per_sec_stdev', 0.0):.2f} req/s")
        print(f"    Response Time:     {aggregate_stats.get('response_time_mean_ms_mean', 0.0):.2f} ± {aggregate_stats.get('response_time_mean_ms_stdev', 0.0):.2f} ms (mean)")
        print(f"    Response Time:     {aggregate_stats.get('response_time_median_ms_mean', 0.0):.2f} ms (median)")
        print(f"    Response Time p95: {aggregate_stats.get('p95_mean', 0.0):.2f} ms")
        print(f"    Response Time p99: {aggregate_stats.get('p99_mean', 0.0):.2f} ms")
        print(f"    Success Rate:      {aggregate_stats.get('success_rate_mean', 0.0):.2f}%")
    ...
    async def run_full_experiment(self):
        ...
        print(f"  Transport: MCP over SSE")
...
async def main():
    ...
    runner = MCPExperimentRunner(config_path)
