...
def calculate_speedup(data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    baseline_throughput = baseline['throughput_req_per_sec_mean']

    # Calculate speedup for each configuration
    for entry in data:
        entry['speedup'] = entry['throughput_req_per_sec_mean'] / baseline_throughput
        entry['efficiency'] = entry['speedup'] / entry['concurrency_level'] * 100  # in percentage
...
def generate_text_report(data: List[Dict[str, Any]], output_file: Path):
    ...
    with open(output_file, 'w') as f:
        ...
        if len(data) >= 2:
            ...
            throughput_increase = last['throughput_req_per_sec_mean'] / first['throughput_req_per_sec_mean']
        ...
        latency_increase = max_latency['response_time_median_ms_mean'] / min_latency['response_time_median_ms_mean']
